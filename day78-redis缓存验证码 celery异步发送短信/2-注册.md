	...
	<div class="box">
        ...
        <div class="register-title">注册路飞学城</div>
				<div class="inp">
					<input v-model="mobile" type="text" placeholder="手机号码" class="user">
					<input v-model="password" type="password" placeholder="登录密码" class="user">
          <input v-model="sms" type="text" placeholder="输入短信验证码" class="user">
          <div id="geetest"></div>
...
export default {
  ...
  methods:{
      registerHandler(){
          ...
          if(!ret){
              this.$alert("对不起，手机号码格式错误！");
              return;
          }

          if(this.sms.length<4 || this.sms.length>6){
              this.$alert("对不起，手机短信格式有无！");
              return;
          }
...
在settings/dev.py配置中数据库下方添加一下代码：
...
"""
自己的账号
"_accountSid": '8aaf07086e0115bb016e818bffe9486d',
"_accountToken": '80b5b7e8e78148279a30a1981c9f4b81',
"_appId": '8aaf07086e0115bb016e818c00494874',
"""    
```

我们将里面的账号信息格外拿出来放到settings的配置文件里去，再导入settings，优化后的libs/yuntongxun/sms.py代码：
...
以后项目上线,肯定使用的是公司已经经过实名认证并且购买短信包的,所以我们修改里面的基本配置信息。settings/dev.py配置文件后面追加配置信息，代码：
...
后端请求基本过程：浏览器发送请求，经由user应用的路由匹配，再由视图处理请求，视图views生成短信验证码，保存验证码，调用CCP短讯sdk给浏览器输入的手机号发送短信，返回响应结果。
