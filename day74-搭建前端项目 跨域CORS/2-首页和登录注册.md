...
在luffyapi目录下创建static和uploads目录
...
#print(sys.path)
#['/home/moluo/Desktop/luffy/luffyapi/luffyapi/apps', '/home/moluo/Desktop/luffy/luffyapi', '/home/moluo/Desktop/luffy/luffyapi'...]
...
注意，pycharm会路径错误的提示(home显示为黄色）。可以鼠标右键设置apps为 mark directory as source root
...
<img src="E:\上课内容\4-路飞学城\day74\2-首页和登录注册.assets\image-20191122094957231.png" alt="image-20191122094957231" style="zoom:50%;" />
...
注意,这里我们使用的ajax的请求域名,配置在了 settings.js中,注意修改成`http://api.luffycity.cn:8000`
...
将前端settings.py中的host更改：

![image-20191122142328390](E:\上课内容\4-路飞学城\day74\2-首页和登录注册.assets\image-20191122142328390.png)
...
公共模型，保存项目的公共代码库目录下luffyapi/utils文件中。
...
class BaseModel(models.Model):
    ...
    #auto_now 当数据每次发生变化时，当前字段使用当前时间戳作为默认值
...
```python
0. 先把现有的数据库导出备份到docs目录，然后清掉数据库中所有的数据表。
	mysqldump -u
...
### JWT的构成
 ...
 JWT构成	https://www.jianshu.com/p/576dbf44b2ae 
